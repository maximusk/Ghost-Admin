...
export default Component.extend({
    ...
    actions: {
        ...
        updateLinesInfo(type, lineNumber) {
            let newLineArray;
            const {line = ''} = this.payload;
            const lineArray = line.split(',');

            if (type === 'add') {
                if (lineArray.indexOf(lineNumber.toString()) !== -1) {
                    return;
                }
                newLineArray = (line + (line ? ',' : '') + lineNumber).split(',');
            } else if (type === 'remove') {
                newLineArray = lineArray.filter(n => n !== lineNumber.toString());
            }

            const sortedLine = newLineArray.sort().join(',');

            this._updatePayloadAttr('line', sortedLine);
        }
    },
